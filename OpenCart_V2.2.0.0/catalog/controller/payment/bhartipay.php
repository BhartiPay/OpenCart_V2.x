...
class ControllerPaymentBhartiPay extends Controller
{
    ...
    public function index()
    {
       ...
       //$return_url = $this->url->link('payment/bhartipay/callback','&hash=' . md5($order_info['order_id']));

        $return_url = $this->url->link('payment/bhartipay/callback');
        ...
        $postdata['button_confirm']= $this->language->get('button_confirm');
         ...
         if(version_compare(VERSION, '2.2.0.0', '>=')) {
            return $this->load->view('payment/bhartipay', $postdata);
        } else {
            if(file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/bhartipay.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/payment/bhartipay.tpl', $postdata);
            } else {
